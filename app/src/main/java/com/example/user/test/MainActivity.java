...
public class MainActivity extends Activity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (arrayListBiodata.size() > 0) {
            for (int i = 0; i < arrayListBiodata.size(); i++) {
                ...
                buttonEdit.get(i).setTag("View");
                buttonEdit.get(i).setText("View");
        ...
        if (view.getId() == R.id.buttonTambahBiodata) {
            ...
            for (int i = 0; i < buttonEdit.size(); i++) {
                ...
                if (view.getId() == buttonEdit.get(i).getId()
                        && view.getTag().toString().trim().equals("View")) {
    ...
    public void getDataByID(int id) {
        String namaEdit = null, alamatEdit = null, jk = null, tgl_lahir = null, no_telp = null;
        ...
        for (int i = 0; i < hashMapBiodata.size(); i++) {
            ...
            jk = hashMapBiodata.get("jk");
            tgl_lahir = hashMapBiodata.get("tgl_lahir");
            no_telp = hashMapBiodata.get("no_telp");
        }
        ...
        final EditText editTgl_lahir = new EditText(this);
        editTgl_lahir.setText(tgl_lahir);
        layoutInput.addView(editTgl_lahir);

        final EditText editJk = new EditText(this);
        editJk.setText(jk);
        layoutInput.addView(editJk);

        final EditText editNo_telp = new EditText(this);
        editNo_telp.setText(no_telp);
        layoutInput.addView(editNo_telp);

        AlertDialog.Builder builderEditBiodata = new AlertDialog.Builder(this);
        builderEditBiodata.setTitle("Biodata");
...
//        });
    ...
    public void tambahBiodata() {
        ...
        final EditText editNo_telp = new EditText(this);
        editNo_telp.setHint("No_telp");
        layoutInput.addView(editNo_telp);

        final EditText edit_jk = new EditText(this);
        edit_jk.setHint("Jenis kelamin");
        layoutInput.addView(edit_jk);


        final EditText editTgl = new EditText(this);
        editTgl.setHint("Tanggal lahir");
        layoutInput.addView(editTgl);
        ...
        builderInsertBiodata.setPositiveButton("Insert",new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                String nama = editNama.getText().toString();
                String alamat = editAlamat.getText().toString();
                String no_telp = editNo_telp.getText().toString();
                String jk = edit_jk.getText().toString();
                String tgl_lahir = editTgl.getText().toString();

                System.out.println("Nama : " + nama + " Alamat : "
                        + alamat);

                sqLiteHelper.tambah_biodata(nama, alamat, tgl_lahir, jk, no_telp);
                        /* restart acrtivity */
                finish();
                startActivity(getIntent());
            }
